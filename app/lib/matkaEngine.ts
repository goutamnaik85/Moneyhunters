...
function getAllJodisFromDigits(digits: number[]): string[] {
  ...
  return Array.from(new Set(jodis)).sort();
}
...
function buildDirectReverseJodis(missing: number[]) {
  ...
  return { direct, reverse: Array.from(new Set(reverse)) };
}
...
function calculateCuts(input: MatkaInput) {
  ...
  const globalCuts = Array.from(new Set([...r15, ...r16, ...r17]));

  return {
    openCuts: Array.from(new Set(openCuts)),
    closeCuts: Array.from(new Set(closeCuts)),
    globalCuts,
    rule151617: Array.from(new Set([...r15, ...r16, ...r17]))
  };
}
...
export function processMatka(input: MatkaInput): MatkaResult {
  ...
  const openDigits = Array.from(new Set(input.openPanel.split('').map(Number)));
  const closeDigits = Array.from(new Set(input.closePanel.split('').map(Number)));
  const allUsed = Array.from(new Set([...openDigits, ...closeDigits]));
  const missingDigits = getMissingDigits(allUsed);

  const { direct, reverse } = buildDirectReverseJodis(missingDigits);
  const initialJodis = Array.from(new Set([...direct, ...reverse])).sort();
